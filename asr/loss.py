# taked from https://github.com/TensorSpeech/TensorFlowASR/blob/main/tensorflow_asr/losses/rnnt_loss.py
...
def compute_rnnt_loss_and_grad_helper(logits, labels, label_length, logit_length):
    ...
    log_probs = tf.nn.log_softmax(tf.cast(logits, dtype=tf.float32))
...
def rnn_transducer_loss(logits, labels, label_length, logit_length, name=None):
